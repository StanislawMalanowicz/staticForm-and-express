...
const debug = require('debug')('app:bandRoutes');
...
function router(nav) {
  bandRouter.route('/:id').get((req, res) => {
    const { id } = req.params;
    // const bandName = collection[id].name;
    // let bandName;
    // const discography = collection[id].albums;
    // let discography;
    const url = 'mongodb://localhost:27017';
    const dbName = 'libraryMusicApp';
    // posible by Promises
    ...
    (async function mongo() {
      ...
      try {
        ...
        const col = await db.collection('albums');

        // eslint-disable-next-line no-shadow
        const collection = await col.find().toArray();
        // eslint-disable-next-line no-console
        console.log('collect mongo: ', collection);
        const bandName = collection[id].name;
        const discography = collection[id].albums;
        res.render('discographyView', {
          bandName,
          discography,
          nav,
        });
